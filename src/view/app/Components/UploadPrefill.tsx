...
onPrefillChange: (preFillFile: File|undefined, preFillValues: SurveySingleItemResponse[]) => void;
...
const UploadPrefill: React.FC<UploadPrefillProps> = (props) => {
    ...
    return (
        <div style={{ width: "fit-content" }} >
            <button style={{ width: "fit-content" }} id="get_file" className="btn btn-secondary"
                onClick={() => openDialogForFileSelection()} > <p>{props.currentSelectFileName} </p></button>
            <input type="file" id="input_file" accept=".json" style={{ display: 'none' }} onChange={(event) => { uploadFileForPreFIll(event); }} />
        </div>
    );
};
